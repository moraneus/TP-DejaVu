...
case class Spec(properties: List[Property]) {
  ...
  def translate(executionMode: Boolean, prePropertySynthesisCode: String): Unit = {
    ...
    writeln(
      s"""
         ...
         |  if (input.length > 1 && input(1).startsWith("[")) {
         |    args += input.slice(1, input.length).mkString(", ")
         |  } else {
         |    // For a case when it is a common DejaVu event
